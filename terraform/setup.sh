...
readonly default_firebase_location=us-central
readonly random_string=$(cat /dev/urandom | tr -dc '[:alnum:]' | fold -w 6 | head -n 1 | tr '[:upper:]' '[:lower:]')
...
while IFS= read -r line; do
    ...
    if [ $var = "TF_VAR_project_id" ]; then
      new_content+="export $var=\"$answer-$random_string\"\n"
    else
